...
import com.skystream.ssheadunit.aap.AapSslContext
import com.skystream.ssheadunit.aap.AapTransport
import com.skystream.ssheadunit.aap.VideoStarvationPolicy
import com.skystream.ssheadunit.utils.AppLog
import com.skystream.ssheadunit.main.BackgroundNotification
import com.skystream.ssheadunit.ssl.SingleKeyKeyManager
...
import com.skystream.ssheadunit.aap.protocol.proto.Input.TouchEvent.PointerAction
...
class CommManager(
     ...
     *
     * Called by [com.skystream.ssheadunit.aap.AapService] in parallel with the
     ...
     * already been called by [com.skystream.ssheadunit.aap.AapProjectionActivity].
    ...
    private fun transportedQuited(isClean: Boolean) {
        ...
        if (settings.killOnDisconnect) {
            context.sendBroadcast(android.content.Intent("com.skystream.ssheadunit.ACTION_FINISH_ACTIVITIES").apply {
                setPackage(context.packageName)
            })
            android.os.Handler(android.os.Looper.getMainLooper()).postDelayed({
                // Stop the foreground service first to remove the notification
                val stopIntent = android.content.Intent(context, com.skystream.ssheadunit.aap.AapService::class.java).apply {
                    action = com.skystream.ssheadunit.aap.AapService.ACTION_STOP_SERVICE
                }
                com.skystream.ssheadunit.aap.AapService.killProcessOnDestroy = true
    ...
    fun sendUpdateUiConfigRequest(left: Int, top: Int, right: Int, bottom: Int) {
        val request = com.skystream.ssheadunit.aap.protocol.messages.UpdateUiConfigRequest(left, top, right, bottom)
        ...
        send(com.skystream.ssheadunit.aap.protocol.messages.VideoFocusEvent(gain = true, unsolicited = true))
    }
