...
import com.skystream.ssheadunit.contract.LocationUpdateIntent
...
class AppThemeManager(
    ...
    private fun update(debounce: Boolean = true) {
        ...
        when (settings.appTheme) {
            Settings.AppTheme.LOCATION -> {
                ...
                isNight = com.skystream.ssheadunit.location.GeofenceLocation
    ...
    companion object {
        ...
        fun reapply(context: Context, settings: Settings) {
            com.skystream.ssheadunit.App.appThemeManager?.stop()
            com.skystream.ssheadunit.App.appThemeManager = null
            if (!isStaticMode(settings.appTheme)) {
                val manager = AppThemeManager(context.applicationContext, settings)
                com.skystream.ssheadunit.App.appThemeManager = manager
                manager.start()
            } else {
